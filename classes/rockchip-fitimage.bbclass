...
python __anonymous () {
        ...
        if ubootenv:
            d.appendVarFlag('do_assemble_fitimage_initramfs', 'depends', ' virtual/bootloader:do_populate_sysroot')
...
# Optional resource (multi) image support
# If RESOURCE_PATH is non-empty, a resource node will be emitted and
# referenced by configurations via the "multi" property
RESOURCE_PATH ?= "resource.img"
...
#
# Emit an optional resource (multi) image section
#
# $1 ... .its filename
# $2 ... node name to use in images section (e.g. resource or resource-1)
# $3 ... path to resource blob
fitimage_emit_section_resource() {

	resource_csum="${FIT_HASH_ALG}"
	resource_sign_algo="${FIT_SIGN_ALG}"
...
fitimage_assemble() {
	...
	#
	# Step 1: Prepare a DTB image section
	#
	...
	if [ -n "${KERNEL_DEVICETREE}" ]; then
		...
		for DTB in ${KERNEL_DEVICETREE}; do
			...
			DTB_NAME=${DTB#*/}
			DTB_NAME=${DTB_NAME%.dtb}

			# Skip DTB if we've picked it up previously
			echo "$DTBS" | tr ' ' '\n' | grep -xq "$DTB_NAME" && continue

			DTBS="$DTBS $DTB_NAME"
			fitimage_emit_section_dtb $1 $DTB_NAME $DTB_PATH
		done
	fi
	...
	fitimage_emit_section_kernel $1 $kernelcount $KERNEL_IMAGE_PATH "$linux_comp"
	...
	#
	# Step 5: Prepare a resource section.
	#
	...
	if [ -n "$DTBS" ]; then
		i=1
		for DTB in ${DTBS}; do
			fitimage_emit_section_config $1 $kernelcount "$DTB" "$ramdiskcount" "$bootscr_id" "$resourcecount" "`expr $i = $dtbcount`"
			i=`expr $i + 1`
		done
	else
		defaultconfigcount=1
		fitimage_emit_section_config $1 $kernelcount "" "$ramdiskcount" "$bootscr_id"  "$resourcecount" $defaultconfigcount
	fi
...
addtask kernel_generate_rsa_keys before do_assemble_fitimage_initramfs after do_compile
		...
		if [ -n "${INITRAMFS_IMAGE}" ]; then
			...
			if [ "${INITRAMFS_IMAGE_BUNDLE}" != "1" ]; then
				...
				if [ -n "${KERNEL_FIT_LINK_NAME}" ] ; then
					...
					ln -snf fitImage-${INITRAMFS_IMAGE_NAME}-${KERNEL_FIT_LINK_NAME} "$deployDir/boot.img"
				fi
			fi
		fi
	fi
...
# do_kernel_generate_rsa_keys should be between do_bundle_initramfs and do_assemble_fitimage_initramfs.
python () {
